<template>
	<div class="device-sonnenbatterie">
		...
		<openwb-base-text-input
			title="API-Token"
			subtype="password"
			:model-value="configuration.api_v2_token"
			@update:model-value="
				updateConfiguration($event, 'configuration.api_v2_token')
			"
			v-if="configuration.variant === 3"
		>
			<template #help>
				API-Token für JSON-API v2. Dieses Token wird benötigt, um die
				Daten abzurufen. Es kann in der Weboberfläche der SonnenBatterie
				unter "Software-Integration" erstellt werden.
			</template>
		</openwb-base-text-input>
	</div>
</template>
